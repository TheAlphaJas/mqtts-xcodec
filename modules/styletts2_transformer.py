...
from .style_encoder import StyleEncoder # Import the new style encoder
import numpy as np
import statistics
...
class StyleTTSDecoder(nn.Module):
    def __init__(self, hp, phoneset_size):
        ...
        # Style Encoder Integration
        self.style_encoder = StyleEncoder(in_channels=80, hidden_channels=128, out_channels=hp.hidden_size)
        # Remove or repurpose spkr_linear if you want to rely solely on style encoder
        # Keeping it for optionality or mixing, but here we'll primarily use style_encoder
        # self.spkr_linear = nn.Linear(512, hp.hidden_size) 

    def forward(self, q, phone, spkr_emb, mel, q_mask, phone_mask):
        # Calculate Style Vector from Mel
        # mel: (B, 80, T)
        style_vec = self.style_encoder(mel) # (B, hidden_size)
        
        # Use style vector as speaker embedding
        spkr = style_vec
    ...
    def inference_topkp_sampling_batch(self, phone, mel, phone_mask, prior=None, output_alignment=False):
        # Compute style from mel input
        style_vec = self.style_encoder(mel)
        spkr = style_vec
        ...
        inps = inp
        #Decode (Logic remains same as original, copied for completeness)
        ...
        for i in range(self.hp.max_output_length):
            ...
            for j in range(self.hp.n_cluster_groups):
                ...
                if self.hp.use_repetition_gating:
                     max_logit = torch.max(logit[:, :self.hp.n_codes], dim=-1)[0]
            ...
            if i == 0:
                past_kvs1 = new_1[:self.hp.dec_nlayers]
            else:
                new_1 = [x[:, -1:] for x in new_1]
                for ii, (p, n) in enumerate(zip(past_kvs1, new_1[:self.hp.dec_nlayers])):
